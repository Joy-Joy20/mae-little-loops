...
export function validateAndNormalizeProductInput(input: unknown): {
  ...
  if (!input || typeof input !== "object") {
    return {
      data: null,
      errors: ["Product payload is missing or invalid."],
    };
  }
  ...
  if (category !== "bouquet" && category !== "keychain") {
    errors.push("Category must be either 'bouquet' or 'keychain'.");
  }
  if (!Number.isFinite(stock) || stock < 0 || !Number.isInteger(stock)) {
    errors.push("Stock must be a whole number equal to or greater than 0.");
  }
  if (!description) {
